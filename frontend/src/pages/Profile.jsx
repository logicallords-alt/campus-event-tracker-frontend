...
const Profile = () => {
  ...
  const handleUpdate = async (e) => {
    e.preventDefault();
    console.log('[Frontend] Updating profile for:', user?.id);
    try {
      const res = await api.put('/api/auth/profile', formData);
      console.log('[Frontend] Profile Update Response:', res.data);
      login(res.data, localStorage.getItem('token')); // Update context user
      toast.success('Profile updated successfully');
    } catch (err) {
      console.error('[Frontend] Profile Update Error:', err.response?.data || err.message);
