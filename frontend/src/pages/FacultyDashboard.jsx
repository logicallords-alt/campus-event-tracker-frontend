...
const FacultyDashboard = () => {
  ...
  const fetchData = async () => {
    try {
      const [statsRes, eventsRes, achRes, globalRes] = await Promise.all([
        axios.get(`${API}/api/events/dashboard-stats`),
        axios.get(`${API}/api/events/all`),
        axios.get(`${API}/api/events/achievements-by-year`),
        axios.get(`${API}/api/events/global`)
      ]);
  ...
  const loadSubmission = async (eventId) => {
    if (submissions[eventId] || loadingSubmission[eventId]) return; // already loaded/loading
    setLoadingSubmission(prev => ({ ...prev, [eventId]: true }));
    try {
      const res = await axios.get(`${API}/api/events/${eventId}/submission`);
      setSubmissions(prev => ({ ...prev, [eventId]: res.data }));
      // Default review tab to photos if available, else certificate
      setReviewTab(prev => ({ ...prev, [eventId]: res.data.event_photos?.length > 0 ? 'photos' : 'certificate' }));
    } catch (err) {
      if (err.response?.status === 404) {
        setSubmissions(prev => ({ ...prev, [eventId]: null })); // mark as checked but empty
      } else {
  ...
  const updateStatus = async (id, status) => {
    try {
      await axios.put(`${API}/api/events/${id}/status`, { status });
    ...
    <div className="main-content">
      ...
      {lightboxImg && (
        ...
        >
          ...
          <img src={lightboxImg && lightboxImg.startsWith('data:') ? lightboxImg : `${API}${lightboxImg}`} alt="Preview" style={{ maxHeight: '90vh', maxWidth: '90vw', borderRadius: '8px' }} />
        </div>
      )}
                            ...
                            <div style={{ display: 'flex', gap: '2rem', flexWrap: 'wrap', marginBottom: '1.5rem' }}>
                              ...
                              {event.event_banner && (
                                <div style={{ minWidth: '140px' }}>
                                  ...
                                  <img
                                    src={event.event_banner?.startsWith('data:') ? event.event_banner : `${API}${event.event_banner}`}
                                    alt="Banner"
                                ...
                                {submissions[event._id] && (
                                  <div style={{ padding: '1.25rem' }}>
                                    ...
                                    {(() => {
                                      ...
                                      const currentTab = reviewTab[event._id] || (hasPhotos ? 'photos' : hasCert ? 'certificate' : 'teamcerts');

                                      return (
                                        <>
                                          {/* Sub-tab bar */}
                                          <div style={{ display: 'flex', gap: '4px', marginBottom: '1rem', background: 'var(--bg-secondary)', borderRadius: '8px', padding: '4px' }}>
                                            {hasPhotos && (
                                              <button
                                              ...
                                              >
                                                <Camera size={13} /> Geotag Photos ({sub.event_photos.length})
                                              </button>
                                            )}
                                            {hasCert && (
                                              <button
                                                  ...
                                                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(130px, 1fr))', gap: '10px' }}>
                                                    {(sub.event_photos || []).map((ph, i) => (
                                                      <div key={i} onClick={() => setLightboxImg(ph)} style={{ position: 'relative', borderRadius: '8px', overflow: 'hidden', cursor: 'zoom-in', border: '1px solid var(--border-color)' }}>
                                                        <img src={ph?.startsWith('data:') ? ph : `${API}${ph}`} alt={`geotag-${i+1}`} style={{ width: '100%', height: '110px', objectFit: 'cover', display: 'block' }} />
                                          ...
                                          {currentTab === 'certificate' && hasCert && (
                                            <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'flex-start', flexWrap: 'wrap' }}>
                                              <div
                                                ...
                                                style={{ cursor: 'zoom-in', borderRadius: '10px', overflow: 'hidden', border: '2px solid var(--border-color)', boxShadow: '0 4px 20px rgba(0,0,0,0.15)' }}
                                              >
                                                <img
                                                  src={sub.certificate_url?.startsWith('data:') ? sub.certificate_url : `${API}${sub.certificate_url}`}
                                                  alt="Certificate"
                                                  onError={e => { e.target.alt = 'PDF — use download button'; e.target.style.padding = '2rem'; }}
                                                  style={{ display: 'block', maxHeight: '280px', maxWidth: '380px', objectFit: 'contain' }}
                                                />
                                              </div>
                                              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', justifyContent: 'flex-end' }}>
                                                <p style={{ fontSize: '0.82rem', color: 'var(--text-secondary)' }}>
                                                  <strong>Student:</strong> {event.student_id?.name}<br />
                                                  <strong>Reg No:</strong> {event.student_id?.reg_no}
                                                </p>
                                                <button
                                                  onClick={() => setLightboxImg(sub.certificate_url)}
                                                  className="btn btn-secondary text-xs py-1 px-3 flex items-center gap-1"
                                                >
                                                  <ImageIcon size={13} /> View Full Size
                                                </button>
                                                <button
                                                ...
                                                {sub.team_member_certificates.map((mc, i) => (
                                                  <div key={i} style={{
                                                    ...
                                                    <div
                                                      ...
                                                      style={{ cursor: 'zoom-in', borderRadius: '8px', overflow: 'hidden', border: '1px solid var(--border-color)', flexShrink: 0 }}
                                                    >
                                                      <img
                                                        src={mc.certificate_url?.startsWith('data:') ? mc.certificate_url : `${API}${mc.certificate_url}`}
                                                        alt={`cert-${i + 1}`}
                                          ...
                                          {!hasPhotos && !hasCert && !hasTeamCerts && (
