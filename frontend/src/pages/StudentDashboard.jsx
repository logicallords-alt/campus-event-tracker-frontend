...
const StudentDashboard = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const [statsRes, eventsRes, achRes, invRes, notifRes] = await Promise.all([
        axios.get(`${API}/api/events/dashboard-stats`, { headers }),
        axios.get(`${API}/api/events/my-events`, { headers }),
        axios.get(`${API}/api/events/my-achievements`, { headers }),
        axios.get(`${API}/api/events/invitations`, { headers }),
  ...
  const markAllRead = async () => {
    try {
      await axios.put(`${API}/api/notifications/read-all`, {}, { headers: authHeader() });
  ...
  const clearAllNotifications = async () => {
    ...
    try {
      await axios.delete(`${API}/api/notifications/clear-all`, { headers: authHeader() });
      setNotifications([]);
  ...
  const markOneRead = async (id) => {
    try {
      await axios.put(`${API}/api/notifications/${id}/read`, {}, { headers: authHeader() });
  ...
  const handleAcceptInvitation = async (id) => {
    try {
      await axios.post(`${API}/api/events/${id}/accept-invitation`);
  ...
  const handleRejectInvitation = async (id) => {
    try {
      await axios.post(`${API}/api/events/${id}/reject-invitation`);
  ...
  const handleSendRequest = async (id) => {
    try {
      await axios.post(`${API}/api/events/${id}/send-request`);
