...
import {
  ...
  Trash2, Trophy, Download, Bell, X, Users, XCircle, AlertCircle, Building
} from 'lucide-react';
          ...
          {achievements.length === 0 ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: '1.25rem' }}>
              {achievements.map((ach, idx) => {
                const isPdf = ach.certificate_url?.startsWith('data:application/pdf');
                return (
                  <div key={idx} style={{
                    background: 'var(--bg-secondary)', border: '1px solid var(--border-color)',
                    ...
                    {ach.certificate_url && (
                      <div style={{ width: '100%', height: '180px', overflow: 'hidden', background: 'var(--bg-primary)', position: 'relative', borderBottom: '1px solid var(--border-color)' }}>
                        {isPdf ? (
                          <iframe
                        ...
                        <span style={{
                          ...
                          background: ach.result === 'Won' ? 'rgba(16,185,129,0.9)' : 'rgba(99,102,241,0.9)',
                          color: '#fff', backdropFilter: 'blur(4px)'
                        }}>
                          {ach.result === 'Won' ? '🏆 Won' : '✅ Participated'}
                        </span>
                      </div>
                    )}

                    {/* No cert but drive link — show placeholder */}
                    {!ach.certificate_url && ach.certificate_drive_link && (
                      <div style={{ width: '100%', height: '80px', background: 'linear-gradient(135deg,#4285F422,#34A85322)', display: 'flex', alignItems: 'center', justifyContent: 'center', borderBottom: '1px solid var(--border-color)' }}>
                        <span style={{ fontSize: '0.82rem', color: '#4285F4', fontWeight: 600 }}>📎 Certificate on Google Drive</span>
                      </div>
                    )}
                    ...
                    <div style={{ padding: '1rem' }}>
                      ...
                      {ach.college_name && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: '5px', fontSize: '0.78rem', color: 'var(--accent-primary)', fontWeight: 600, marginBottom: '4px' }}>
                          <Building size={12} /> {ach.college_name}
                        </div>
                      )}
                      <p style={{ color: 'var(--text-secondary)', fontSize: '0.78rem', marginBottom: '0.6rem' }}>
                        {new Date(ach.event_date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}
                      </p>

                      {/* Student info */}
                      <div style={{ fontSize: '0.82rem', color: 'var(--text-secondary)', marginBottom: '0.75rem', display: 'grid', gridTemplateColumns: 'auto 1fr', gap: '2px 10px' }}>
                        <span style={{ fontWeight: 600, color: 'var(--text-primary)' }}>Name</span>   <span>{ach.student_name}</span>
                        <span style={{ fontWeight: 600, color: 'var(--text-primary)' }}>Reg No</span> <span>{ach.reg_no}</span>
                        {ach.team_name && <><span style={{ fontWeight: 600, color: 'var(--text-primary)' }}>Team</span><span>{ach.team_name}</span></>}
                        {ach.is_team_member && (
                          <><span></span><span style={{ color: 'var(--accent-primary)', fontWeight: 600, fontSize: '0.75rem', display: 'flex', alignItems: 'center', gap: '4px' }}><Users size={11} /> Team Member</span></>
                        )}
                      </div>
                        ...
                        {ach.certificate_drive_link && (
                          <a
                            ...
                            style={{
                              ...
                              padding: '6px 0', borderRadius: '8px', fontSize: '0.78rem', fontWeight: 700,
                              background: 'linear-gradient(135deg,#4285F4,#34A853)', color: '#fff',
                              textDecoration: 'none', cursor: 'pointer'
                            }}
                          >
                            🔗 Open Certificate (Google Drive)
                          </a>
                        )}
                      </div>
                    </div>
