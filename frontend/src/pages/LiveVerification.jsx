...
import { Camera, MapPin, CheckCircle, Trash2, Upload, AlertCircle, Calendar } from 'lucide-react';
...
const LiveVerification = () => {
  ...
  useEffect(() => {
    const fetchEvent = async () => {
      try {
        const res = await axios.get(`${API}/api/events/my-events`);
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      await axios.post(`${API}/api/events/${id}/live-verification`, payload, {
